...
export type InputCheckboxProps = ViewProps & {
  formik?: FormikValues;
  value?: boolean;
  id: string;
  disabled?: boolean;
...
const InputCheckbox = (props: InputCheckboxProps) => {
  ...
  const handleChange = useCallback(() => {
    if (Object?.keys(formik?.values ?? {})?.find((key) => key === id)) {
      formik?.setFieldValue(id, !formik?.values?.[id]);
    } else if (setChecked) {
      setChecked && setChecked(!isChecked);
    }
  }, [setChecked, isChecked, formik?.values]);
  ...
  return (
    ...
    >
      <AtomWrapper  customCSS={wrapperCSS}>
        <Pressable
          disabled={disabled}
          accessibilityRole="checkbox"
          accessibilityState={{ disabled, checked: formik?.values[id] ?? isChecked }}
          style={[
            styles.root,
            style,
            isChecked && styles.checked,
            !!colorChecked && {
              backgroundColor: formik?.values[id] ?? isChecked ? colorChecked : undefined,
              borderColor: formik?.values[id] ?? isChecked ? colorChecked : colorBoxChecked,
            },
            disabled && styles.checkedAndDisabled,
          ]}
          onPress={!disabled ? handleChange : () => {}}
          {...inputCSS}
        >
          {(formik?.values[id] || isChecked) && (
            <Image
              source={{
                uri: checkmarkBase64,
              }}
